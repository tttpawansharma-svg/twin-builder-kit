...
import { Step9Links } from "./wizard/steps/Step9Links";
import { DigitalTwinProfile } from "@/types/digitalTwin";
import { ChevronLeft, ChevronRight, Download, Sparkles, Loader2, Crown, Building2, User, ArrowLeft } from "lucide-react";
import { toast } from "sonner";
import { useDigitalTwin } from "@/contexts/DigitalTwinContext";
import { useAuth } from "@/contexts/AuthContext";
...
You represent them professionally in conversations, staying authentic, strategic, and human.`;
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#0A1929] via-[#0D2137] to-[#0A1929] flex items-center justify-center">
        <div className="text-center space-y-4">
          <div className="relative">
            <div className="w-16 h-16 border-4 border-cyan-500/20 border-t-cyan-400 rounded-full animate-spin mx-auto"></div>
            <User className="w-6 h-6 text-cyan-400 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2" />
          </div>
          <div>
            <h3 className="font-semibold text-lg text-white mb-2">Loading Your Digital Twin</h3>
            <p className="text-slate-300">Preparing your professional persona...</p>
          </div>
        </div>
      </div>
    );
  ...
  if (showOutput) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#0A1929] via-[#0D2137] to-[#0A1929] py-12 px-4">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-8">
            <div className="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-r from-cyan-500 to-teal-400 rounded-full mb-4 shadow-lg shadow-cyan-500/30">
              <Crown className="w-8 h-8 text-white" />
            </div>
            <h1 className="text-4xl font-bold text-white mb-3">Digital Twin Complete</h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <Card className="p-8 shadow-lg border border-slate-200 bg-white backdrop-blur-sm">
              <div className="flex items-center gap-3 mb-6">
                <div className="w-10 h-10 bg-primary/10 rounded-lg flex items-center justify-center">
                  <Sparkles className="w-5 h-5 text-primary" />
                </div>
                <div>
                  <h2 className="text-xl font-semibold text-slate-800">AI Persona Initialization</h2>
                  <p className="text-sm text-slate-600">Copy this prompt to initialize your AI assistant</p>
                </div>
              </div>
              <div className="bg-slate-50 border border-slate-200 p-6 rounded-xl font-mono text-sm whitespace-pre-wrap max-h-96 overflow-y-auto text-slate-700">
                {generatePersona()}
              </div>
            </Card>
            ...
            <Card className="p-8 shadow-lg border border-slate-200 bg-white backdrop-blur-sm">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="text-xl font-semibold text-slate-800">Complete Profile Data</h2>
                  <p className="text-sm text-slate-600">Full JSON structure for integration</p>
                </div>
              </div>
              <div className="bg-slate-50 border border-slate-200 p-6 rounded-xl overflow-auto max-h-96">
                <pre className="text-xs text-slate-700">{JSON.stringify(data, null, 2)}</pre>
          ...
          <div className="flex gap-4 justify-center mt-8">
            <Button 
              onClick={downloadJSON} 
              className="px-8 py-6 text-base font-semibold bg-gradient-to-r from-primary to-secondary hover:from-primary/90 hover:to-secondary/90 shadow-lg hover:shadow-xl transition-all duration-200 border-0"
            >
              <Download className="w-5 h-5 mr-3" />
              Download JSON Profile
            </Button>
            <Button
              ...
              variant="outline"
              className="px-8 py-6 text-base font-semibold border-2 border-slate-300 hover:bg-slate-50 text-slate-700 transition-colors duration-200"
            >
              Create Another Twin
            </Button>
          </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0A1929] via-[#0D2137] to-[#0A1929] py-12 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-r from-cyan-500 to-teal-400 rounded-2xl shadow-lg shadow-cyan-500/30 mb-6">
            <User className="w-10 h-10 text-white" />
          </div>
          <h1 className="text-5xl font-bold bg-gradient-to-r from-cyan-400 to-teal-300 bg-clip-text text-transparent mb-4">
            Digital Twin Architect
          </h1>
          ...
          {digitalTwin && (
            <div className="mt-6 inline-flex items-center gap-2 px-4 py-3 bg-teal-500/10 border border-teal-500/30 rounded-xl shadow-sm">
        ...
        <Card className="p-8 shadow-2xl border-0 bg-green-50 rounded-3xl">
          <StepIndicator currentStep={currentStep} totalSteps={totalSteps} stepTitles={STEP_TITLES} />
          ...
          <div className="flex justify-between items-center pt-8 mt-8 border-t border-slate-200">
            <Button 
              ...
              variant="outline" 
              disabled={currentStep === 0}
              className="px-6 py-3 h-auto font-semibold border-2 border-slate-300 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-slate-50 text-slate-700 transition-colors duration-200"
            >
              <ChevronLeft className="w-5 h-5 mr-2" />
              Previous Step
            </Button>
            ...
            <Button 
              onClick={handleNext} 
              className="px-8 py-3 h-auto font-semibold bg-gradient-to-r from-primary to-secondary hover:from-primary/90 hover:to-secondary/90 shadow-lg hover:shadow-xl transition-all duration-200 min-w-[140px] disabled:opacity-50 disabled:cursor-not-allowed border-0"
              disabled={isSaving}
            >
              {isSaving ? (
                <Loader2 className="w-5 h-5 animate-spin" />
